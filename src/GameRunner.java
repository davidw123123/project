...
public class GameRunner {
    public static void main(String[] args) {
        ...
        while (userMove || computerMove) {//checks if both user's move are true
            System.out.print("Choose between hit or stand or use your ability: ");
            String response = s.nextLine();
            if ((user.abilities.length()>1)&&(!response.equalsIgnoreCase("hit")&&!response.equalsIgnoreCase("stand"))){// checks if user's response is not hit nor stand and they have an ability

                if (response.equalsIgnoreCase(user.abilities)) {//if response is equal to their ability nad if not, asks user to choose between hit or stand
            ...
            } else if (!response.equalsIgnoreCase("hit")&&!response.equalsIgnoreCase("stand")){// if user
                System.out.println("You don't have any abilities! Choose between hit or stand");
                response = s.nextLine();
            }

            if ( response.equalsIgnoreCase("hit"))
            {// calls hit method if response is equal to hit and stand otherwise
            ...
            { //computer uses ability after 2 turns
            ...
            if (computerMove)
            {
                if (((Math.abs(computer.total - computer.goal) < 5) || (Math.abs(computer.total - computer.goal) > computer.goal))) { // if computer total's difference from goal is less than 5, it'll stand
